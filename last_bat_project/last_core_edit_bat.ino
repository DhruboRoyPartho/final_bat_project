...
void Task1(void *pvParameters) {
    while (true) {
        if (xSemaphoreTake(xMutex, portMAX_DELAY)) {  // Acquire mutex for display control
            ...
            //     print_wb_temp();
            // }
            // Insert display control code here
            xSemaphoreGive(xMutex);  // Release mutex after display update
        }
...
void Task2(void *pvParameters) {
    while (true) {
        if (xSemaphoreTake(xMutex, portMAX_DELAY)) {  // Acquire mutex for temperature reading
            temp_measure();
            
            // Insert temperature measurement code here
            xSemaphoreGive(xMutex);  // Release mutex after measurement
        }
        vTaskDelay(10 / portTICK_PERIOD_MS);
    }
}
...
void setup() {
    ...
    // Create a mutex to control access to shared resources
...
// Phase control
void phase_control() {
    if(hour >= limit_hour && minute >= limit_min && second >= limit_sec && alarmable && (selected_mode == 2)){
        // writeIntToEEPROM(EEPROM, flag1, 0);
        // writeIntToEEPROM(EEPROM, flag2, 0);
        delay(10);
        warning_section(5);
    ...
    else if(hour >= limit_hour && minute >= limit_min && second >= limit_sec && (selected_mode == 1)){
        selected_phase++;
        if(selected_phase >= 5){
            // writeIntToEEPROM(EEPROM, flag1, 0);
            // writeIntToEEPROM(EEPROM, flag2, 0);
