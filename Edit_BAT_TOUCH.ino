...
const byte phase_duration_hour[4] = {4, 10, 12, 10};    //4 10 12 10
const byte phase_duration_min[4] = {0, 0, 30, 0};   //0 0 30 0
...
// Relay control
void blower_start(){
    digitalWrite(13, HIGH);   // blo_relay
}
void blower_stop(){
    digitalWrite(13, LOW);    // blo_relay
}
void damper_open(){
    digitalWrite(12, LOW);    // dam_relay1
    digitalWrite(14, HIGH);      // dam_relay2
}
void damper_close(){
    digitalWrite(14, LOW);    // dam_relay2
    digitalWrite(12, HIGH);   // dam_relay1
}
...
void temp_control() {
    ...
    if(cur_clock_flag){
        // control temp here
        if((byte)tempF[0] < phase_temp[selected_phase - 1][hour][0]){
            // Blower start
            blower_start();
        }
        else{
        ...
        if((byte)tempF[1] > phase_temp[selected_phase - 1][hour][1]){
            // damper open
            damper_open();
        }
        else{
...
void home_touch(){
    ...
    if(pressed && home_key[2].contains(t_x, t_y)){
        Serial.println("home_key 3 pressed");
        home_key[2].press(true);
    }else{
        home_key[2].press(false);
    }
    ...
    if(home_key[2].justReleased()) home_key[2].drawButton();
    if(home_key[2].justPressed()) home_key[2].drawButton(true);
    delay(10);
...
void homeKeypad() {
    ...
    // Reset button
    home_key[2].initButton(&tft, NORMAL_KEY_X + 95, NORMAL_KEY_Y - NORMAL_KEY_H - NORMAL_KEY_SPACING_Y, 45, NORMAL_KEY_H, TFT_WHITE, TFT_BLUE, TFT_WHITE, "0", KEY_TEXTSIZE);
    home_key[2].drawButton();
...
// !!!!!!UI- For special character print
////////////////////////////////////////
